...
\hs Table~\ref{tab:cyber_security_aggregated_results} presents the mean classification results for the Cyber Security Attacks dataset, all evaluated models obtain very close Accuracy values, ranging from approximately $0.331$ to $0.340$. Linear SVC and Logistic Regression achieve the highest reported mean Accuracy, both around $0.340$, while \varname{KFCClassifier-CombinedClassifier} remains very close with a mean Accuracy of $0.339$. This indicates that the KFC-based classification model is competitive in terms of overall Accuracy, but it does not clearly outperform the strongest linear baseline models.
...
\hs The regression results are reported as mean $\pm$ standard deviation over five random seeds. The evaluated datasets include Abalone, California Housing Price, and Wind Turbine. The reported metrics are MAPE, RMSE, $R^2$, and the average value of the true target in the test split.
...
\hs Table~\ref{tab:abalone_aggregated_results} presents the mean regression results for the Abalone dataset, \varname{KFCRegressor-GradientCOBRA} obtains the highest reported mean $R^2$ value of $0.560$ and the lowest RMSE value of $2.143$ among the evaluated models. \varname{MixCOBRARegressor}, \varname{SVR}, and \varname{GradientCOBRA} also perform competitively, with mean $R^2$ values of $0.555$, $0.551$, and $0.550$, respectively. Although \varname{SVR} achieves the lowest MAPE value, the results based on RMSE and $R^2$ indicate that the clusterwise KFC configuration provides the best overall regression fit under this experimental setting.
